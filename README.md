...
腳本會建立 release app、放入匹配版本的 usage helper，並安裝到 `~/Applications/DevOrchestratorBar.app`（可用 `DEV_ORCHESTRATOR_APP_DIR` 指定其他使用者 Applications 目錄）。啟動後會直接打開「Dev Orchestrator 用量」視窗，右上角選單列也會保留快速入口；點擊圖示會直接開啟用量面板，滑過圖示會顯示目前用途與五小時總量。這是 `LSUIElement` 選單列 App，因此不會在 Dock 顯示一般應用程式圖示。
